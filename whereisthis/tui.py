...
def cache_path(dir: str, hidden: bool) -> Path:
    key = f"{Path(dir).resolve()}|hidden={hidden}|v={CACHE_VERSION}"
    digest = hashlib.sha1(key.encode()).hexdigest()
    cache_dir = Path.home() / ".cache" / "whereisthis"
...
class whereisthis(App):
    ...
    def load_index(self):
        cache = cache_path(self.directory, self.show_hidden)
        ...
        if cache.exists():
            try:
                ...
                if (
                    ...
                    and data.get("directory") == str(Path(self.directory).resolve())
                    and data.get("hidden") == self.show_hidden
